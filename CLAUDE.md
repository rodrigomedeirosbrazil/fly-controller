...
- All four thresholds are editable in the web portal (Configurações → Proteção Térmica) and persist in NVS, so the numbers above are only the factory values. Anything that draws or documents a derating range must read `Settings`, never hardcode these.
...
Static assets (HTML/CSS/JS) are **gzipped at build time** by `scripts/gen_web_assets.py` into `$BUILD_DIR/generated/WebAssetsGz.h` and served with `Content-Encoding: gzip` — 107 KB → 30 KB, which matters on a board at ~87% of its app partition. The files under `src/WebServer/Pages/` remain the source of truth but are **not `#included` by any translation unit**, so the uncompressed text is never linked in; add a new asset by listing it in the script's `ASSETS` table. Pages built at runtime through `renderPage()` link `/config.css` rather than inlining it (inlining built the whole stylesheet inside an Arduino `String` on every render).
...
The **telemetry page** (`/telemetry`) polls `/api/telemetry` every 1 s and mirrors buzzer sounds in the browser via Web Audio API. The `buzzer` field in the JSON response is an **array** of up to 8 `BeepEvent` entries (ring buffer, oldest→newest: `seq`, `freq`, `onMs`, `offMs`, `reps`, `layer`, `active`) — `layer` is 0 for a queued event, 1 for the persistent state layer. The browser plays fresh queued events (`seq > bzLastSeq`) immediately and toggles a looping oscillator on state transitions only (repeated `active:true`/`active:false` for an already-running/stopped state is ignored); an event pauses the state loop and resumes it afterward. A speaker toggle in the status bar unlocks the `AudioContext` (browser autoplay policy) and controls mute.

The `/api/telemetry` response also includes a `powerAlert` object (`{seq, causes: [...]}`) driven by the `PowerAlert` component. The page reads only `causes[]`: the limiting tile turns red, the power readout gains a `DISPONÍVEL xx %` chip and the throttle bar gains a ceiling tick. There is no banner, so `seq` has no consumer on the page any more — it still drives the component's own 10 s re-fire (beeps). See `PowerAlert/` for the component and `PowerAlertLogic.h` for the host-testable timing logic.

**Telemetry page layout.** Six bands in a fixed vertical order filling `100dvh` — status, battery, instruments, throttle, drawer handle, navigation — sized in `fr` and `clamp()` with exactly one media query (`orientation`). Nothing is ever added to or removed from that stack, so no alert can move a number the pilot is reading: a fault disarm shows as a chip in space the status bar already reserves, and every secondary reading lives in an overlay drawer. Missing data collapses the layout (no current → one hero cell; no `powerKw` → the instrument row goes 3 → 2 columns) instead of printing `N/A`. Battery, motor and ESC are SVG dials (`stroke-dasharray` on a `viewBox`, 270° sweep) — no pixel size, only a proportion, which is what survives an orientation change. Motor/ESC full scale and reduction band come from a one-shot `GET /api/config/thermal`, not from the 1 Hz payload. The hero's voltage cell shows the pack total by default and toggles to per-cell on tap, persisted in `localStorage` under `tpVoltMode` (through the guarded `storeGet`/`storeSet` helpers — an unguarded `localStorage` read at top level used to kill the whole page where site data is blocked). The unit switches with the mode (`V` / `V/cél`) so the two numbers can never be confused. Per-cell prefers the BMS minimum cell and otherwise divides the pack voltage by a hardcoded 14, marked with a tilde — the firmware has no support for other pack sizes, so this is deliberately a constant and not a setting.

The `sessionSec` field in `/api/telemetry` is a **flight-time counter** (shown in the Telemetry status bar, reset from the drawer) that survives disarm/re-arm cycles within a power cycle: it counts only while `isArmed && motorRunning`, pauses on disarm or motor stop, resumes from the same value, and is RAM-only (resets on boot). Pure decision logic lives in `src/HourMeter/HourMeterLogic.h` (host-tested in `test/HourMeterLogicTest.cpp`); `src/HourMeter/HourMeter.cpp` is the Arduino wrapper that also owns the persistent `hourMeterSec` total (motor-run seconds, saved to NVS). `POST /api/session/reset` (PIN-guarded, like every write route) zeroes the session counter via a deferred flag — the reset is applied on the next `loop()` tick, never from the web-server task. The Telemetry page's reset button prompts for the PIN on a 403 and caches it in `sessionStorage` (`cfgPin`), since `/telemetry` is self-contained and does not include the config pages' PIN field.
