...
The stream buffer is not decoration. A plain shared buffer raced: `flush()` captured a length, blocked in `Update.write()`, then zeroed the cursor — discarding everything the Bluedroid task staged during the block, after the accepted offset and the running CRC had already counted it. The whole image transferred, `commitAllowed()` passed, and `Update.end(true)` failed on a short partition. A stream buffer is safe for one writer and one reader without a mutex, which is exactly this pair. The handle is *detached* on abort or finish and freed a full loop iteration later by `releaseIfRetired()` — freeing inside `abort()` would trade the data race for a use-after-free, since abort runs on the loop task while a BLE callback may be part-way through `stage()`. `begin()` reclaims a handle still in quarantine rather than allocating a second one, so only one buffer ever exists.
