...
int RC_min[6] = {916, 1060, 976, 960, 1056, 1116};
int RC_mid[6] = {1492, 1446, 1424, 1398, 1374, 1460};
...
void pwmPIN_to_port()
{
  for (int i = 0; i < num_ch; i++)
  {
    ...
    if (pwmPIN[i] >= 0 && pwmPIN[i] <= 7)
      pwmPIN_port[i] = 2; // pin belongs to PCINT2_vect  (PORT D)
    else if (pwmPIN[i] >= 8 && pwmPIN[i] <= 13)
      pwmPIN_port[i] = 0; // pin belongs to PCINT0_vect (PORT B)
    ...
    if (pwmPIN[i] == 0 || pwmPIN[i] == A0 || pwmPIN[i] == 8)
      pwmPIN_reg[i] = 0b00000001;
    else if (pwmPIN[i] == 1 || pwmPIN[i] == A1 || pwmPIN[i] == 9)
      pwmPIN_reg[i] = 0b00000010;
    else if (pwmPIN[i] == 2 || pwmPIN[i] == A2 || pwmPIN[i] == 10)
      pwmPIN_reg[i] = 0b00000100;
    else if (pwmPIN[i] == 3 || pwmPIN[i] == A3 || pwmPIN[i] == 11)
      pwmPIN_reg[i] = 0b00001000;
    else if (pwmPIN[i] == 4 || pwmPIN[i] == A4 || pwmPIN[i] == 12)
      pwmPIN_reg[i] = 0b00010000;
    else if (pwmPIN[i] == 5 || pwmPIN[i] == A5 || pwmPIN[i] == 13)
...
{ //  this function will run if a pin change is detected on portB
  ...
  { //  run through each of the channels
    ...
    { //  if the current channel belongs to portB
      ...
      if (prev_pinState[i] == 0 && PINB & pwmPIN_reg[i])
      ...
      else if (prev_pinState[i] == 1 && !(PINB & pwmPIN_reg[i]))
...
{ //  this function will run if a pin change is detected on portC
  ...
  { //  run through each of the channels
    ...
    { //  if the current channel belongs to portC
      ...
      if (prev_pinState[i] == 0 && PINC & pwmPIN_reg[i])
      {                                       // and the pin state has changed from LOW to  HIGH (start of pulse)
        prev_pinState[i] = 1;                 //  record pin state
        pwmPeriod[i] = pciTime - pwmTimer[i]; //  calculate the time period, micro sec, between the current and previous pulse
        pwmTimer[i] = pciTime;                // record the  start time of the current pulse
      }
      else if (prev_pinState[i] == 1 && !(PINC & pwmPIN_reg[i]))
      {                                // or the pin state has changed from HIGH to LOW  (end of pulse)
        prev_pinState[i] = 0;          //  record pin state
        PW[i] = pciTime - pwmTimer[i]; //  calculate the duration of the current pulse
        pwmFlag[i] = HIGH;             //  flag that new data is available
...
{ //  this function will run if a pin change is detected on portD
  ...
  { //  run through each of the channels
    ...
    { //  if the current channel belongs to portD
      ...
      if (prev_pinState[i] == 0 && PIND & pwmPIN_reg[i])
      {                                       // and the pin state has changed from LOW to  HIGH (start of pulse)
        prev_pinState[i] = 1;                 //  record pin state
        pwmPeriod[i] = pciTime - pwmTimer[i]; //  calculate the time period, micro sec, between the current and previous pulse
        pwmTimer[i] = pciTime;                // record the  start time of the current pulse
      }
      else if (prev_pinState[i] == 1 && !(PIND & pwmPIN_reg[i]))
      {                                // or the pin state has changed from HIGH to LOW  (end of pulse)
        prev_pinState[i] = 0;          //  record pin state
        PW[i] = pciTime - pwmTimer[i]; //  calculate the duration of the current pulse
        pwmFlag[i] = HIGH;             //  flag that new data is available
...
float RC_decode(int CH)
{
  ...
  { // If the RC channel is outside of failsafe tolerances  (10-330hz and 500-2500uS)
    if (CH > size_RC_failsafe)
      CH_output = 0; //  and if no failsafe position has been defined, set output to neutral
    else
      CH_output = RC_failsafe[i]; // or if defined set the failsafe position
  }
  else
