...
void ControllerWebServer::startAP() {
    ...
    // Live buzzer volume preview — applies the volume at runtime (not persisted)
    // and plays a short beep so the user hears the new level while adjusting.
    server.on("/api/buzzer/preview", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        if (!request->hasParam("volume")) {
            request->send(400, "text/plain", "Parâmetro volume ausente"); return;
        }
        long volume = request->getParam("volume")->value().toInt();
    ...
    AsyncCallbackJsonWebHandler* saveSystemHandler = new AsyncCallbackJsonWebHandler(
        ...
        [](AsyncWebServerRequest *request, JsonVariant &json) {
            ...
            if (!doc.containsKey("buzzerVolume")) {
                request->send(400, "text/plain", "Campo buzzerVolume ausente");
                return;
            }
    ...
    server.on("/api/telemetry", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        if (motorTempSrc) {
            signals["motorTempSrc"] = motorTempSrc;
        }
